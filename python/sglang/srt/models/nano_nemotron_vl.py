...
from sglang.srt.configs.nano_nemoron_vl import NemotronH_Nano_VL_V2_Config
...
class NemotronH_Nano_VL_V2(nn.Module):
    ...
    def get_image_feature(self, items: List[MultimodalDataItem]) -> torch.Tensor:
        # items[i].feature is expected to be a tensor of pixel data
        device = next(self.language_model.parameters()).device
        pixel_values = torch.cat([item.feature for item in items], dim=0).to(device)
        image_features = self.extract_feature(pixel_values)
        # Flatten over batch to a single sequence for embed_mm_inputs to slice
