...
from sglang.multimodal_gen.configs.models.encoders.qwen3 import Qwen3Config
...
def default_preprocess_text(prompt: str) -> str:
    return prompt


def default_postprocess_text(outputs, _text_inputs):
    # Use final hidden layer as embeddings; pad sequences to max length in batch
...
@dataclass
class NemotronTextPipelineConfig(PipelineConfig):
    ...
    # Text encoding stage (loader will overwrite arch fields from ckpt config)
    text_encoder_configs: tuple[EncoderConfig, ...] = field(
        default_factory=lambda: (Qwen3Config(),) # todo is doenst matter since we overrite it ...
    )

    text_encoder_precisions: tuple[str, ...] = field(default_factory=lambda: ("bf16",))

    preprocess_text_funcs: tuple[Callable[[str], str], ...] = field(
        default_factory=lambda: (default_preprocess_text,)
    )

    postprocess_text_funcs: tuple[Callable[[str], str], ...] = field(
        default_factory=lambda: (default_postprocess_text,)
    )
